...
{	ConnectionFrame connectionFrame;
	...
	public Player (ConnectionFrame connectionFrame, IgsGoFrame gf, IgsStream in, PrintWriter out,
		PlayDistributor pd)
	{
	...
	*/
	...
	{	if (!HaveSize) // get board size first of all
		...
		{
            if (GF.isIgnoreNextPass()) {
                GF.setIgnoreNextPass(false);
            } else {
                GF.setpass();
            }
			return;
	...
	*/
    ...
    {	StringParser p;
        ...
        p.parseword();
        String Komi = Rule.normalizeWonBy(Rule.getRule(GF.getCountRule()).getKomi());
