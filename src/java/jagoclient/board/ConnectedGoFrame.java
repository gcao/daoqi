...
public class ConnectedGoFrame extends GoFrame {
    ...
    protected MultiLineLabel BlackLabel, WhiteLabel;
    protected Menu FileMenu, Options;
    protected Panel CommentPanel;
    protected Panel cp = new MyPanel(); // container of AllComments and SendField
    protected TextArea AllComments;
    CardLayout CommentPanelLayout;
    protected CheckboxMenuItem KibitzWindow, Teaching, Silent;
    protected CheckboxMenuItem chineseCountRule, jihuoziRule, japaneseRule;
    ...
    public ConnectedGoFrame
        ...
        */
        ...
        M.add(score);
        //score.add(chineseCountRule = new CheckboxMenuItemAction(this, Global.resourceString("Chinese_Rule")));
        //score.add(jihuoziRule = new CheckboxMenuItemAction(this, Global.resourceString("Ji_Huo_Zi_Rule")));
        //score.add(japaneseRule = new CheckboxMenuItemAction(this, Global.resourceString("Japanese_Rule")));
        //score.addSeparator();
        score.add(new MenuItemAction(this, Global.resourceString("Adjourn")));
        score.addSeparator();
        ...
        // Add the timer label
        if (!TimerInTitle) {
            SimplePanel btl = new SimplePanel(TL = new MyLabel(Global.resourceString("Start")), 80, new MyLabel(""), 20);
            bpp.add(btl);
        ...
        if (BigTimer) {
            ...
            p.setBackground(Global.ControlBackground);
            p.add("West", WhiteLabel = new MultiLineLabel(Global.resourceString("White")));
            WhiteLabel.setFont(Global.SansSerif);
            WhiteLabel.setBackground(p.getBackground());
            WhiteLabel.setAlignment(Label.CENTER);
            BL = new BigTimerLabel();
            //cp.add("North", BL);
            p.add("Center", BL);
            p.add("East", BlackLabel = new MultiLineLabel(Global.resourceString("Black")));
            BlackLabel.setFont(Global.SansSerif);
            BlackLabel.setBackground(p.getBackground());
            BlackLabel.setAlignment(Label.CENTER);
            cp.add("North", p);
        }
        // Navigation panel
        IB = createIconBar();
        ...
        focusManager = new FullScreenKeyboardFocusManager(SendField);
        setVisible(true);
        repaint();
    }
