...
{	ConnectedGoFrame CGF;
	...
	*/
    ...
    /**
     * mandatorily set a move for the opposite player
     * @param i
     * @param j
     */
    public synchronized void setOppositeMove(int i, int j) {
        if (Pos.haschildren()) return;
		if (P.color(i,j)!=0) return;
		if (captured==1 && capturei==i && capturej==j &&
			GF.getParameter("preventko",true)) return;
        //System.out.println("setOppositeMove():MainColor="+MainColor);
        //System.out.println("setOppositeMove():MyColor="+MyColor);
        if (MainColor > 0) {
            ((GoFrame)GF).black(i,j);
        } else {
            ((GoFrame)GF).white(i,j);
        }
    }

    /**
     * mandatorily set my move.
     * @param i
     * @param j
     */
    public synchronized void setSelfMove(int i, int j) {
        if (Pos.haschildren()) return;
		if (P.color(i,j)!=0) return;
		if (captured==1 && capturei==i && capturej==j &&
			GF.getParameter("preventko",true)) return;
        //System.out.println("setSelfMove():MainColor="+MainColor);
        //System.out.println("setSelfMove():MyColor="+MyColor);
        if (MainColor > 0) {
            ((GoFrame)GF).black(i,j);
        } else {
            ((GoFrame)GF).white(i,j);
