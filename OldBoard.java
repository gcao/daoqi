...
public class Board extends Canvas
	...
	public int getMoveNumberToShow(int move) {
		move ++;
		int numToShow = 0;
		if (Range > 0) {
			if (move >= Range) {
	...
	{
		font = GF.boardFont();
		largeFont = new Font(font.getFamily(), font.getStyle(), font.getSize() * 2);
...
// !!! We alow for suicide moves
	...
	void update1(Graphics g, int i, int j) {
		...
		if (lasti == origi && lastj == origj && showlast) {
			...
			if (GF.lastNumber() || Range != 0) {
				...
				if (P.number(origi, origj) > 0) {
					...
					int numToShow = getMoveNumberToShow(P.number(origi, origj));
					if (numToShow > 0) {
		...
		//if (P.color(origi, origj) != 0 && Range > 0 && (lasti < 0 || P.number(lasti, lastj) - P.number(origi, origj) < Range))
		{
			if (P.color(origi, origj) > 0)
				g.setColor(Color.white);
			else
	...
	private void drawLastMoveMark(Graphics g, int i, int j, int x, int y) {
		if (GF.bwColor()) {
			...
			g.setColor(Color.red);
		g.drawLine(x + D / 2 - D / 6, y + D / 2, x + D / 2 + D / 6, y + D / 2);
		g.drawLine(x + D / 2, y + D / 2 - D / 6, x + D / 2, y + D / 2 + D / 6);
	}
	...
	{
		if (ActiveImage == null) return;
		//if (Range > 1 && lasti >= 0) updateall();
		if (Range < 0 && lasti >= 0) updateall();
		try {
